...
final class TaskRepository
{
    ...
    {
        $sql = 'SELECT id, title, due_date, due_time FROM tasks WHERE owner_id = :owner_id AND status = :status ORDER BY CASE WHEN due_date IS NULL THEN 3 WHEN due_date = :today THEN 0 WHEN due_date = :tomorrow THEN 1 ELSE 2 END, due_date ASC, CASE WHEN due_time IS NULL OR due_time = \'\' THEN 1 ELSE 0 END, due_time ASC, id DESC';
    ...
    public function getTimedOpenTasksByOwnerAndDate(int $ownerId, string $date): array
    {
        $sql = 'SELECT id, title, due_time FROM tasks WHERE owner_id = :owner_id AND status = :status AND due_date = :date AND due_time IS NOT NULL AND due_time != \'\' ORDER BY due_time ASC';
        $stmt = $this->db->pdo()->prepare($sql);
        $stmt->execute(['owner_id' => $ownerId, 'status' => 'open', 'date' => $date]);
        return $stmt->fetchAll();
    }
    ...
    public function getTodayAlertTasksByOwner(int $ownerId, string $today): array
    {
        $sql = 'SELECT id, title, due_time FROM tasks WHERE owner_id = :owner_id AND status = :status AND due_date = :due_date AND due_time IS NOT NULL AND due_time != \'\' ORDER BY due_time ASC, id DESC';
        $stmt = $this->db->pdo()->prepare($sql);
        $stmt->execute(['owner_id' => $ownerId, 'status' => 'open', 'due_date' => $today]);
        return $stmt->fetchAll();
    }
