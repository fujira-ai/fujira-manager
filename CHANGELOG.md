...
### Fixed
- ノイズワード除去で「やること確認」「今日やること確認」のような入力まで意図せず削ってしまう問題を修正
- 「やること」「今日やること」は、後続に空白がある場合のみ先頭ノイズとして除去するよう改善
- 「今から」「とりあえず」「まずは」は従来どおり先頭ノイズとして除去しつつ、入力補完の自然さを維持

---

## v1.3.20-dev - 2026-03-23

### Changed
- タスク完了後の返信文を改善し、次にやるべき今日のタスク1件を提示するよう変更
- 今日の残タスクがある場合は、時刻あり優先で次の1件を表示するよう改善
- 今日の残タスクが0件の場合は、「今日のタスクはすべて完了です！🎉」を表示するよう改善

---

## v1.3.19-dev - 2026-03-23

### Changed
- `cron/push_tomorrow_reminder.php` の18時通知を改善
- 今日の未完了タスクと明日の予定を1通の通知にまとめるよう変更
- 今日未完了 → 明日予定の順で表示し、一日の締めと翌日の準備を同時に確認できるよう改善
- 各セクションは最大3件表示し、超過時は「他にも〜」で補足するよう調整
- どちらか一方のみ存在する場合は該当セクションのみ表示
- 今日未完了タスクがある場合のみ「今日」で確認できる導線を表示
- 両方0件の場合は通知しないよう調整
...
### Changed
- `cron/push_midday_brief.php` の昼通知を、今日の未完了タスク整理に特化
- 明日の予定表示を削除し、13時通知の役割を「今日の未完了確認」に整理
- 今日の未完了タスクを最大3件表示し、時刻ありタスクを優先表示するよう改善
- 期限なしタスクは補足として表示し、件数超過時は「他にも未完了タスクがあります。」を表示するよう改善
- 今日の未完了タスクが0件の場合は通知しないよう調整

---

## v1.3.16-dev - 2026-03-23

### Changed
- 10分前通知（pre-alert）の文面を改善
- 時刻をタイトル行の先頭に移動し、`10:00 歯医者` のように一目で分かる形式に調整
- ヘッダーに ⏰ を追加し、通知一覧での視認性を向上
- 通知末尾に「今日」から完了できる導線を追加
- タスク抽出・重複防止など既存の通知ロジックは維持
...
### Fixed
- 初回体験フロー STEP 0 でユーザーの最初の入力が破棄される問題を修正
- STEP 0 の専用 continue ブロックを廃止し、最初の入力を通常どおり処理するよう改善

### Changed
- タスク登録後のチュートリアルヒントを step 0 / 1 / 2 に対応するよう調整
- 「今日」コマンド実行時に、STEP 0 ユーザーへタスク登録を促すヒントを追加
- 初回体験フローを「入力を止める方式」から「返信にヒントを重ねる方式」へ改善
...
### Fixed
- 「今のは」「さっきのは」で始まる直前タスク修正コマンドが、通常コマンド判定に吸われて無反応になる問題を修正
- 直前タスク修正の判定を通常コマンド処理より前に移動し、自然な修正入力に正しく応答するよう改善

### Improved
- 直前タスク修正処理にデバッグログを追加し、分岐・解析・更新の追跡をしやすく改善
- 例外発生時に無反応にならず、エラーメッセージを返すよう改善

---

## v1.3.11-dev - 2026-03-23

### Changed
- 直前タスク修正コマンドで「です」なしの自然な入力にも対応
- `今のは今日` `今のは10時` `今のは今日10時` のような入力で日時修正できるよう改善
- `です` `に変更` `にして` `してください` などの末尾表現を吸収するよう自然言語パースを改善
...
### Changed
- 日付指定がなく時刻だけを含む入力を、今日の予定として扱うよう自然言語解析を改善
- `9時に日本リアライズ` や `日本リアライズ 9時` のような入力で、期限なしではなく当日予定として登録できるよう改善
- `明後日` や `来週` を含む入力は今日補完の対象外とし、既存の明示日付ロジックを優先するよう調整
